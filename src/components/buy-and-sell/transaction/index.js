...
export default function BuyAndSellTransaction(props) {
  const { activePage, activeCrypto, setActiveCrypto } = props;

  console.log('activeCrypto', activeCrypto, props);
        ...
        <div>
          <Tabs
            ...
            onChange={(e, newValue) => {
              ...
              setActiveCrypto(NavigationItems[newValue].key);
            }}
    ...
    return (
      <BoxLayout header="2. Enter amount">
        <div
          ...
          style={{
            flexDirection: activePage === 'buy' ? 'column' : 'column-reverse',
          }}
        >
  ...
  const renderSummary = () => {
    return (
      <BoxLayout header={`${activePage === 'buy' ? 4 : 3}. Order summary`}>
        <div className={`${s.summaryHead} center flexColumn`}>
          <div>{activePage === 'buy' ? 'BUYING' : 'SELLING'}</div>
  ...
  return (
    <div className={s.root}>
      {renderCurrecyTabs()}
      {renderEnterAmount()}
      {activePage === 'buy' && renderWalletForBuy()}
      {renderSummary()}
      <Button
        variant="primary"
        title={`${activePage} ${NavigationItems[currencyIndex].label}`}
        className={s.proceedBtn}
        titleClassName={s.proceedBtnTitle}
      />
      {activePage === 'sell' && renderWalletForSell()}
    </div>
  );
}
